...
class LowRankTensorCompletionTNN(object):
    ...
    def log_f(self, x,theta, lam):
        x = np.abs(x)
        y = lam*np.log(theta*x+1)/np.log(theta+1)
        return y
    
    def log_sg(self,x,theta, lam):
        x = np.abs(x)
        y = lam*theta/((theta*x+1)*np.log(theta+1))
        return y
    
    def log_p(self, x,theta, lam):
        G = 0
        for k in range(x.shape[2]):
            u, s, v = np.linalg.svd(x[:,:,k], full_matrices = 0)
            for sigma in s:
                G = G + self.log_f(sigma,theta, lam)
        return G
    
    def log_svt(self,img,theta, lam):
        u, s, v = np.linalg.svd(img, full_matrices = 0)
        th = self.log_sg(s,theta, lam)
        s = s - th
        s[s < 0] = 0
        X_hat = np.matmul(np.matmul(u, np.diag(s)), v)
        return X_hat
    
    def GLTC_LOG(self,dense_tensor, sparse_tensor, theta, lam, maxiter, mask):
        ...
        rse1 = mt.relative_error(self.x_miss,self.ground_truth)
        count = 0
    
        #while np.linalg.norm(rse1) > self.epsilon:
        for iters in range(self.max_iter):
                for k in range(dim0):
                    Z_hat = self.ten2mat(X[:, :, :, k] + T[:, :, :, k] / self.rho, k)
                    Z_hat = self.log_svt(Z_hat, theta, lam)
                    Z[:, :, :, k] = self.mat2ten(Z_hat, dim, k)
                    var = self.ten2mat(self.rho * Z[:, :, :, k] - T[:, :, :, k], k)
                    if k == 0:
                        var0 = self.mat2ten(np.matmul(inv(self.beta * np.matmul(D1.T, D1) + self.rho * np.eye(dim1)), var), dim, k)
                    elif k == 1:
                        var0 = self.mat2ten(np.matmul(inv(self.beta * np.matmul(D2.T, D2) + self.rho * np.eye(dim2)), var), dim, k)
                    else:
                        var0 = Z[:, :, :, k] - T[:, :, :, k] / self.rho
                ...
                rse[iters] = rse1
                rankp[iters] = self.log_p(tensor_hat, theta, lam )
    ...
    def rankpenalty_tnn(self,img, alpha, theta):
        P = 0
        count = 0
        for k in range(img.shape[2]):
            u, s, v = np.linalg.svd(img[:,:,k], full_matrices = 0)
    ...
    def svt_tnn(self, mat, tau, theta):
    ...
    def GLTC_TNN(self, dense_tensor, sparse_tensor, alpha, beta, rho, theta, maxiter, mask, validation = 1):
        """Main function of the GLTC-Geman."""
        dim0 = sparse_tensor.ndim
        dim1, dim2, dim3 = sparse_tensor.shape
        dim = np.array([dim1, dim2, dim3])
        ...
        for iters in range(maxiter):
            for k in range(dim0):
                ...
                Z_hat = self.svt_tnn(Z_hat, alpha / rho, theta)
                Z[:, :, :, k] = self.mat2ten(Z_hat, dim, k)
            ...
            rse1 = mt.relative_error(tensor_hat,self.ground_truth)
            rse[iters] = rse1
            rankp[iters], count[iters] = self.rankpenalty_tnn(tensor_hat, alpha, theta)
            self.x_hat = tensor_hat.copy()
            tsc = cst.tsc(self.x_hat, self.ground_truth, self.ten_ones, self.mask_indices)
            cost = tu.loss_func(self.x_hat, dense_tensor)
    ...
    def save_solution_scans_and_history(self, iteration, suffix, folder): 
        ...
        elapsed_time_output['cost'] = cost_arr
    ...
    def save_solution_scans(self, iteration, suffix, folder): 
        mrd.draw_original_vs_reconstructed_rim_z_score2(image.index_img(self.ground_truth_img, 0), image.index_img(self.x_hat_img,0), image.index_img(self.x_miss_img, 0), self.title + " Iteration: " + str(iteration),
                        0, self.observed_ratio, 0, 0, 2, coord=None, folder=self.images_mr_folder_iteration, iteration=-1)
    ...
    def complete(self):
        ...
        #self.GLTC_LOG(self.ground_truth, self.x_miss, self.theta, self.lam, self.max_iter, self.mask_indices)
        self.GLTC_TNN(self.ground_truth, self.x_miss, self.alpha, self.beta, self.rho, self.theta, self.max_iter, self.mask_indices)
