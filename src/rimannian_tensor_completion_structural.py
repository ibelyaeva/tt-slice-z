...
# section of riemannian computations     
    ...
    def define_train_operations(self):
        ...
        #self.eta_cond = tf.cond((tf.abs(self.theta) >= self.zero_point1_tf), self.get_conj_dir, self.get_conj_dir)
    ...
    def get_draw_timepoints(self):
        ...
        if len(self.random_ts) == 1:
            self.middle_ts1 = self.random_ts[0] 
        else:
            self.middle_ts1 = self.random_ts[1] 
            
        if len(self.random_ts) == 1:    
            self.middle_ts2 = self.random_ts[0]
        else:
            self.middle_ts2 = self.random_ts[int(len(self.random_ts)/2.0)]
        
        if len(self.random_ts) == 1:
            self.max_ts = self.random_ts[0]
        else:
    ...
    def save_medata(self):
        ...
        md_output['rank'] = str(self.max_tt_rank)
